    ...
    <main className="min-h-screen bg-black px-4 py-6 sm:px-6 sm:py-8">
      <section className="mx-auto w-full max-w-2xl space-y-6">
        <div className="rounded-2xl border border-gray-800 bg-[#111111] p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-400">People who follow this user.</p>
        </div>

        {isLoading ? <p className="text-sm text-gray-400">Loading followers...</p> : null}
        ...
        {!isLoading && !error && data && data.length === 0 ? (
          <div className="rounded-2xl border border-gray-800 bg-[#111111] p-6 text-sm text-gray-400">No followers yet.</div>
        ) : null}
